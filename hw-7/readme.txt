...
Для теста использовал browser-sync в качестве локального сервера:
установка на node npm: install -g browser-sync
запуск сервера browser-sync start --server --files "./**/*.*" --no-notify

к сожалению не все успел реализовать что хотел, но вроде как основные задачи выполнил, на задачу со звездочкой к сожалению времени не осталось
